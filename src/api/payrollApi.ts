...
import { Payroll, PayrollCreatePayload, PayrollStatusUpdatePayload,GeneratePayrollPayload } from '../types/payroll';
...
generateAll(data: GeneratePayrollPayload) {
  return axiosClient
  ...
  getById(payroll_id: number) {
    return axiosClient.get<Payroll>(`${URL}/${payroll_id}`).then((res) => res.data);
  },

  // Cập nhật trạng thái bảng lương (VD: từ "pending" → "paid")
  updateStatus(payroll_id: number, data: PayrollStatusUpdatePayload) {
    return axiosClient.put<Payroll>(`${URL}/${payroll_id}/status`, data).then((res) => res.data);
  },

  // Xoá bảng lương
  delete(payroll_id: number) {
    return axiosClient.delete<{ message: string }>(`${URL}/${payroll_id}`).then((res) => res.data);
  },
};
